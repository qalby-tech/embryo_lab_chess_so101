...
def frames(path):
    out = subprocess.run(["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries",
                          "stream=nb_frames,r_frame_rate", "-of", "csv=p=0", path],
                         capture_output=True, text=True, check=True).stdout.strip().split(",")
    num, den = out[0].split("/")
    return int(out[1]), float(num) / float(den)

def clip(src, start_f, n_f, fps, caption, outcome, ok, dst, tmp):
    ...
    vf = (f"trim=start_frame={start_f}:end_frame={start_f + n_f},setpts=PTS-STARTPTS,"
          ...
          f"drawtext=fontfile={FONT}:textfile={cap}:x=14:y=10:fontsize=22:fontcolor=white,"
          f"drawtext=fontfile={FONT}:textfile={res}:x=14:y=38:fontsize=22:fontcolor={color}")
...
def main():
    ...
    args = ap.parse_args()
    clip_dir = os.path.join(os.path.dirname(args.out), "clips"); os.makedirs(clip_dir, exist_ok=True)
    reel = []
    with tempfile.TemporaryDirectory() as tmp:
        for spec in args.specs:
            video, log, family = spec.rsplit(":", 2)
            eps = LINE.findall(open(log, errors="ignore").read())
            total, fps = frames(video)
            ...
            for i, instr, k, err, extra in eps[: args.limit]:
                ...
                clip(video, i * per, per, fps, f"{family.upper()}  |  {instr}", outcome, ok, dst, tmp)
                made[k].append(dst)
